...
@Component({
  ...
  templateUrl: './topup-request.page.html',
  styleUrls: ['./topup-request.page.scss'],
})
export class TopupRequestPage implements OnInit {
  ...
  addimei() {
    if (this.imeidetail.value.imei != '') {
      ...
      }&Status=TOPUP&dealer=${localStorage.getItem('userId')}`;
  ...
  submit() {
    ...
    for (let i = 0; i < selectdata.length; i++) {
      arr.push({
        ...
        updateddate: null,
        topupvalidity: this.imeidetail.value.validityperiod,
      });
    }

    const url =
      'https://mvt.apmkingstrack.com/fleettracking/esim/saveMobileEsimTopupRequest?companyid=apm&dealerid=' +
