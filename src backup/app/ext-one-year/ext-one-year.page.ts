...
@Component({
  selector: 'app-ext-one-year',
  templateUrl: './ext-one-year.page.html',
  styleUrls: ['./ext-one-year.page.scss'],
})
export class ExtOneYearPage implements OnInit {
  ...
  addimei() {
    if (this.imeidetail.value.imei != '') {
      ...
      }&Status=EXTEND&dealer=${localStorage.getItem('userId')}`;
  ...
  submit() {
    ...
    const url =
      'https://mvt.apmkingstrack.com/fleettracking/esim/saveMobileEsimExtendOneYearRequest?companyid=apm&dealerid=' +
